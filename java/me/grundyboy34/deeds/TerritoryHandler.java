...
import me.grundyboy34.deeds.savedata.PlayerSaveData;
...
public class TerritoryHandler {
	private static SaveData saveData; // Save data in one single point, ready to be stored to disk
	...
	static void setSaveData(SaveData set) {
		saveData = set;
	}
	...
	static boolean canPlayerEditChunk(World world, EntityPlayer player, Chunk chunk) {
		...
		DeedSaveData faction = getFactionByChunkPos(world.provider.getDimension(), chunkPos);
		...
		return isPlayerMemberOfFaction(faction.getID(), player, player.capabilities.isCreativeMode); // If you are you
																										// get to edit
																										// the chunk.
	}

	public static DeedSaveData getFactionByChunkPos(int worldID, ChunkPos pos) {
		// Main.console("Getting faction by chunk position...");

		Iterator<Entry<UUID, DeedSaveData>> it = saveData.getFactionMapIterator();
		Entry<UUID, DeedSaveData> entry;
		...
		while (it.hasNext()) {
			...
			if (entry.getValue().getDimensionID() != worldID) {
				continue;
			} // Wrong dimension
	...
	static boolean isPlayerMemberOfFaction(UUID factionID, EntityPlayer player, boolean isCreative) {
		...
		if (isCreative) {
			return true;
		} // Creative mode players are members of ALL factions. Leaders, too.

		if (saveData.getPlayerMap().containsKey(playerID)) {
			// Is a known player, so checking their data
			PlayerSaveData playerData = saveData.getPlayerMap().get(playerID);
			...
			PlayerSaveData playerData = new PlayerSaveData(playerID);
	...
	public static DeedSaveData startFactionInChunk(World world, EntityPlayer player, Chunk chunk) {
		// Safeties
		if (player == null) {
			return null;
		} // Who're you?
		...
		Deeds.console("Starting a new faction for player " + player.getName() + " (ID " + playerID + ")...");

		// Step 1, create the faction
		DeedSaveData faction = new DeedSaveData(UUID.randomUUID(), world.provider.getDimension(),
				player.getName() + "'s faction");
		...
		if (saveData.getPlayerMap().containsKey(playerID)) {
			...
			Deeds.console("...player is known, using their profile...");

			playerData = saveData.getPlayerMap().get(playerID);
		} else {
			// Is not yet known, so adding them now
			Deeds.console("...player is not known, making a new profile...");

			playerData = new PlayerSaveData(playerID);
			saveData.getPlayerMap().put(playerID, playerData);
		}

		Deeds.console("...adding player as member and leader...");
	...
	public static void checkUpkeep() {
		...
		int maxTick = Deeds.getUpkeepTickMax();
		...
		int upkeep = Deeds.getUpkeepCost();

		// Alright, let's do this.
		Deeds.console("Checking upkeep for all controlled chunks. Cost per chunk: " + upkeep + " VALUABLES ...");

		// Faction by faction
		Iterator<Entry<UUID, DeedSaveData>> facIt = saveData.getFactionMapIterator();
		Entry<UUID, DeedSaveData> facEntry;

		while (facIt.hasNext()) {
			facEntry = facIt.next();

			Deeds.console("...checking faction " + facEntry.getKey() + "...");
			...
			if (cost > valuables) {
				...
				Deeds.console("...faction cannot pay upkeep. Disabling protection...");
			} else {
				// Seems affordable
				facEntry.getValue().setValuables(valuables - cost);
				facEntry.getValue().setProtected(true);

				Deeds.console("...faction successfully paid " + cost + " in upkeep. Enabling protection...");
			}
		}

		Deeds.console("...done checking upkeep.");
	}
	...
	public static DeedSaveData getFactionFromLeader(EntityPlayer player) {
		if (player == null) {
			return null;
		}
	...
	public static boolean isChunkOwnedByFaction(UUID factionID, Chunk chunk) {
		...
		DeedSaveData faction = saveData.getFactionMap().get(factionID);

		if (faction == null) {
			return false;
		} // No faction with that ID exists.
	...
	public static boolean isChunkAdjacentToTerritory(World world, UUID factionID, Chunk chunk) {
		...
		if (tempChunk != null && faction.isChunkClaimed(tempChunk.getChunkCoordIntPair())) {
			return true;
		} // Checks out.
	...
	public static void checkChunkControl() {
		...
		int maxTick = Deeds.getChunkTickMax();
		...
		Deeds.console("Refreshing chunk control strength by " + Deeds.getChunkControlRefreshValue() + " now...");
		...
		while (it.hasNext()) {
			...
			Deeds.console("...checking faction " + entry.getValue().getName() + " (ID " + entry.getKey() + ")...");

			Iterator<ChunkSaveData> chunkIt = entry.getValue().getChunkList().iterator();
			ChunkSaveData chunk;

			while (chunkIt.hasNext()) {
				chunk = chunkIt.next();

				if (chunk.strength < Deeds.getChunkControlMax()) {
					chunk.strength += Deeds.getChunkControlRefreshValue();

					if (chunk.strength > Deeds.getChunkControlMax()) {
						// Making sure we don't spill over
						chunk.strength = Deeds.getChunkControlMax();
					}
				}
		...
		Deeds.console("... done refreshing chunk control.");
	}

	// See if enough players of that faction are online to be able to attack it.
	public static boolean canAttackFaction(World world, DeedSaveData faction) {
		if (faction == null) {
			return false;
		} // ...who're you trying to attack?

		// Step 1, can factions be attacked at all?
		if (!Deeds.allowAttacks()) {
			return false;
		}

		// Step 2, do we care whether or not anyone is online?
		if (!Deeds.doesPlayerNeedToBeOnline()) {
			return true;
		} // No requirement, so have at it.
		...
		int membersRequired = Deeds.getRequiredPlayerAmount(); // In percent
	...
	private static ArrayList<UUID> getMembersFromFaction(UUID factionID) {
		...
		Iterator<Entry<UUID, PlayerSaveData>> it = saveData.getPlayerMapIterator();
		Entry<UUID, PlayerSaveData> entry;
	...
	public static void disbandFaction(World world, UUID factionID, Entity dropEntity) {
		...
		if (faction == null) {
			...
			Deeds.console("Warning: Someone tried to disband a faction that doesn't seem to exist. ID: " + factionID);
			return;
		}

		// Disband this faction, kick all members out, tell them about it and scatter
		// their valuables at the feet of the entity specified
		Deeds.console("Disbanding faction " + faction.getName() + " (ID " + factionID + ")...");

		// Step 1, kick all members out and tell them about it
		Deeds.sendMessageToFactionMembers(world, factionID, "Disbanding faction " + faction.getName() + "!");

		Iterator<Entry<UUID, PlayerSaveData>> playerIt = saveData.getPlayerMapIterator();
		Entry<UUID, PlayerSaveData> playerData;
		...
		while (playerIt.hasNext()) {
			...
			if (factionID.equals(playerData.getValue().getLeadership())) {
				// Oh hello, leader.
				Deeds.console("...removing leader " + playerData.getKey() + "...");
			...
			if (playerData.getValue().getMemberList().contains(factionID)) {
				// Hello, member.
				Deeds.console("...removing member " + playerData.getKey() + "...");
				playerData.getValue().getMemberList().remove(factionID);
			}
		}

		Deeds.console("...removing the faction itself...");
		...
		if (faction.getValuables() <= 0) {
			Deeds.console("...no VALUABLES to scatter. Done removing faction.");
			return;
		}

		Deeds.console("...scattering " + faction.getValuables() + " VALUABLES...");
		...
		Deeds.console("...done disbanding faction.");
	}
}*/
