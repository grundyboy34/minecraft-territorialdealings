...
public class DeedMap extends ItemMap {
	public DeedMap() {
	...
	private boolean isMapReady(ItemStack stack) {
		...
		if (!stack.getTagCompound().getBoolean("initiated")) {
			return false;
		}
	...
	public void updateMapData(World worldIn, Entity viewer, MapData data) {
		if (worldIn.provider.getDimensionType().getId() == data.dimension && viewer instanceof EntityPlayer) {
			...
			for (int unknownInt = mapX - mapY + 1; unknownInt < mapX + mapY; ++unknownInt) {
				this.updateMapData_2(worldIn, viewer, data, mapX, mapY, mapZ, unknownInt, mapScale, mapCenterX,
						mapCenterZ, mapinfo, flag);
			}
		}
	}

	private void updateMapData_2(World worldIn, Entity viewer, MapData data, int mapX, int mapY, int mapZ,
			int unknownInt, int mapScale, int mapCenterX, int mapCenterZ, MapData.MapInfo mapdata$mapinfo,
			boolean bUnknown) {
			...
			for (int unknownInt_2 = mapZ - mapY - 1; unknownInt_2 < mapZ + mapY; ++unknownInt_2) {
				if (unknownInt >= 0 && unknownInt_2 >= -1 && unknownInt < 128 && unknownInt_2 < 128) {
					...
					if (!chunk.isEmpty()) {
						...
						SaveData save = DeedSaveHandler.instance().getCurrentSave();
						boolean hasProtections = false;
						boolean isOwner = false;
						boolean hasPermissions = false;
						boolean shouldHaveDeedColor = false;
						if (save != null) {
							if (viewer instanceof EntityPlayer) {
								EntityPlayer player = (EntityPlayer) viewer;
								ChunkSaveData chunkData = save.getChunkSaveData(new SerializedChunkPos(chunk.xPosition, chunk.zPosition));
								if (chunkData != null) {
									hasProtections = chunkData.hasProtections();
									isOwner = chunkData.getOwner().equals(player.getUniqueID());
									hasPermissions = isOwner;
									shouldHaveDeedColor = true;
									if (!hasPermissions) {
										DeedSaveData deedData = save.getDeed(chunkData.getOwner());
										if (deedData != null && player.getGameProfile() != null) {
						...
						if (worldIn.provider.getHasNoSky()) {
							...
							if ((l3 >> 20 & 1) == 0) {
								multiset.add(
										Blocks.DIRT.getDefaultState()
												.withProperty(BlockDirt.VARIANT, BlockDirt.DirtType.DIRT).getMapColor(),
										10);
							} else {
								multiset.add(Blocks.STONE.getDefaultState()
										.withProperty(BlockStone.VARIANT, BlockStone.EnumType.STONE).getMapColor(),
										100);
							}
							...
							for (int i4 = 0; i4 < mapScale; ++i4) {
								for (int j4 = 0; j4 < mapScale; ++j4) {
										...
										label542: {
											while (true) {
												...
												iblockstate = chunk.getBlockState(
														blockpos$mutableblockpos.setPos(i4 + i3, blockHeight, j4 + j3));
											...
											if (blockHeight > 0 && iblockstate.getMaterial().isLiquid()) {
												...
												while (true) {
													IBlockState iblockstate1 = chunk.getBlockState(i4 + i3, l4--,
															j4 + j3);
													++k3;
									...
									MapColor chunkColor = iblockstate.getMapColor();
									//System.out.println(Boolean.toString(shouldHaveDeedColor) + ":" + Boolean.toString(isOwner) + ":" + Boolean.toString(hasPermissions));
									if (shouldHaveDeedColor) {								
										if (isOwner) {
											chunkColor = hasProtections ? MapColor.BLUE : MapColor.CYAN;
										} else if (hasPermissions) {
											chunkColor = hasProtections ? MapColor.GREEN : MapColor.GRASS;
										} else {
											chunkColor = hasProtections ? MapColor.RED : MapColor.PINK;
										}
									}
									multiset.add(chunkColor);
								}
							}
						}

						k3 = k3 / (mapScale * mapScale);
						double d2 = (dUnknown - d0) * 4.0D / (mapScale + 4)
								+ ((unknownInt + unknownInt_2 & 1) - 0.5D) * 0.4D;
						int i5 = 1;
						...
						MapColor mapcolor = Iterables.getFirst(Multisets.<MapColor>copyHighestCountFirst(multiset),
								MapColor.AIR);
						...
						if (unknownInt_2 >= 0 && i2 * i2 + j2 * j2 < mapY * mapY
								&& (!flag1 || (unknownInt + unknownInt_2 & 1) != 0)) {
